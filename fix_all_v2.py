...
print("=== ПОВНА МОДИФІКАЦІЯ ДОКУМЕНТА v2 ===\n")
...
for i in range(sources_start + 1, len(doc.paragraphs)):
    ...
    if has_url:
        ...
        if match:
            ...
            # Видаляємо дату звернення
            clean_text = re.sub(r'\s*\(дата звернення:?\s*[\d\.]+\)\.?', '', text)

            # Видаляємо URL: та сам URL
            clean_text = re.sub(r'\s*URL:\s*', ' ', clean_text)
            clean_text = re.sub(re.escape(url), '', clean_text)
            ...
            # Виправляємо подвійні крапки та пробіли
            result = re.sub(r'\s*\.\s*\.', '.', result)
            result = re.sub(r'\s+', ' ', result)

            para.text = result
            print(f"  {source_num}. {clean_text[:60]}... URL: {url[:30]}...")
    else:
        # Без URL
        text = text.rstrip('.')
        result = f"{source_num}. {text}."
        para.text = result
