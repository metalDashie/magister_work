...
export default function EditAttributePage() {
  ...
  useEffect(() => {
    ...
    loadData()
  }, [_hasHydrated, isAuthenticated, user, router, attributeId])

  const loadData = async () => {
    try {
      setLoading(true)
      const [attributeRes, categoriesRes] = await Promise.all([
        api.get(`/attributes/${attributeId}`),
        api.get('/categories?flat=true'),
      ])

      const attr = attributeRes.data
      setAttribute(attr)
      setFormData({
        name: attr.name || '',
        slug: attr.slug || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.patch(`/attributes/${attributeId}`, {
      ...
      setErrors({
        submit: error.response?.data?.message || 'Failed to update attribute',
      })
    } finally {
      setSaving(false)
    }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-600">Update attribute settings</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 space-y-6">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Changing the slug may break existing product attribute mappings
          </p>
        </div>
        ...
        <div className="space-y-3">
          ...
          <label className="flex items-center gap-3">
            <input
              ...
              className="w-4 h-4 text-primary-600 border-gray-300 rounded focus:ring-primary-500"
            />
            <span className="text-sm text-gray-700">Visible (show on product page)</span>
          </label>
        </div>
